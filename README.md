...
1. **配置 AI 服务**
   - 进入「设置」页面
   - 选择 AI 提供商（阿里云百炼 DashScope / vLLM 本地）
   - 填写 API 端点、密钥和模型名称
   - 点击「测试连接」验证配置
2. **添加搜索文件夹**
   - 进入"索引"页面
...
应用支持两种后端 AI 服务，详细协议请参阅 [API 协议说明](api-protocol.md)：

- **阿里云百炼 DashScope** — 托管 API，无需 GPU，按量付费
- **vLLM 本地部署** — 使用 [Qwen3-VL-Embedding-2B-AWQ-4bit](https://huggingface.co/LifetimeMistake/Qwen3-VL-Embedding-2B-AWQ-4bit) 等多模态模型

部署指南请参阅 [LLM 部署指南](llm-deployment.md)。
